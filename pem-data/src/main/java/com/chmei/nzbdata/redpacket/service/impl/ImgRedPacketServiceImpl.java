...
import com.chmei.nzbdata.redpacket.service.IImgRedPacketService;
...
public class ImgRedPacketServiceImpl extends BaseServiceImpl implements IImgRedPacketService {
	 ...
	 * log對象
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(ImgRedPacketServiceImpl.class);
	 ...
	 */
	@Override
	public void saveImgRedPacketInfo(InputDTO input, OutputDTO output) throws NzbDataException {
		LOGGER.info("ImgRedPacketServiceImpl.saveImgRedPacketInfo, input::" + input.getParams().toString());
		Map<String, Object> params = input.getParams();
		try{
			int redPacketImgCount = (int) params.get("redPacketImgCount");
			if(redPacketImgCount < 50){
				output.setMsg("红包个数不能小于50个");
				return;
			}
			String redPacketImgType = (String) params.get("redPacketImgType"); // 发红包类型
			...
			if("0".equals(outputDTO.getCode())){
				...
				BigDecimal redPacketImgMoneyCount = (BigDecimal) params.get("redPacketImgMoneyCount");
				params.put("redPacketImgId", getSequence()); // 获取id
				params.put("redPacketImgUserId", params.get("memberAccount")); // 获取用户id
				int count = getBaseDao().insert("ImgRedPacketMapper.saveImgRedPacketInfo", params);
				boolean flag = false;
				if (count > 0) {
					Map<String, Object> map = new HashMap<>();
					// 插入红包成功,扣除相应的款项,钱包,红包,广告钱包
					switch (redPacketImgType) {
						case WALLET_TYPE: // 钱包扣款
							double walletAmount = Double.valueOf(walletBalance) - redPacketImgMoneyCount.doubleValue();
							...
							if (i <= 0) {
								...
								walletMoneyInfo.put("walletInfoMoney", redPacketImgMoneyCount);
								walletMoneyInfo.put("walletInfoFrom", "发布图文广告");
								getBaseDao().insert("WalletMoneyInfoMapper.saveWalletMoneyInfo", walletMoneyInfo);
							}
							break;
						case RED_PACKET_TYPE: // 红包扣款
							double hbAmount = Double.valueOf(redEnveBalance) - redPacketImgMoneyCount.doubleValue();
							...
							if (j <= 0) {
								...
								redRecord.put("redPacketInfoMoney", redPacketImgMoneyCount);
								redRecord.put("redPacketInfoFrom", "发布图文广告");
								getBaseDao().insert("RedRecordMoneyInfoMapper.saveRedRecordMoneyInfo", redRecord);
							}
							break;
						case ADVERSING_TYPE: // 广告钱包扣款
							double advertisingAmount = Double.valueOf(advertisingFee) - redPacketImgMoneyCount.doubleValue();
							...
							if (k <= 0) {
								...
								adRecord.put("advertisingInfoMoney", redPacketImgMoneyCount);
								adRecord.put("advertisingInfoFrom", "发布图文广告");
					...
					if (flag) {
						...
						String id = (String) params.get("redPacketImgId");
						int i = getBaseDao().delete("ImgRedPacketMapper.delImgRedPacketInfo", id);
						if(i <= 0){
					...
					// 发布成功,直接进行红包的分配:
					List<Double> doubles = gradRedPacket(redPacketImgCount, redPacketImgMoneyCount.doubleValue());
					int i = this.insertListRedPacketImgInfo(doubles, (Long) params.get("redPacketImgId"));
...
//						sendBackRedPacketMoney(stringObjectMap); // 红包退回定时任务
		...
		timer.schedule(new TimerTask() { // 红包发布成功，执行定时任务
			...
			public void run() {
				...
				Map<String, Object> map_ = (Map<String, Object>) getBaseDao().queryForObject(
						"ImgRedPacketMapper.queryRedPacketMoneyStock", params);
				if (null != map_) {
							...
							switch (redPacketType) {
								case WALLET_TYPE: // 钱包退回
									...
									if (i <= 0) {
										...
										// 删除过期的红包信息
										getBaseDao().delete("ImgRedPacketMapper.delRedPacketMoneyStock", params);
										// 钱包退回金额记录:
										Map<String, Object> walletMoneyInfo = new HashMap<>();
										walletMoneyInfo.put("walletInfoId", getSequence());
										walletMoneyInfo.put("walletInfoAddOrMinus", "+");
		...
		if(i > 0){
			...
			if(Optional.ofNullable(packet).isPresent()){
				...
				if(stock < count && stock == 1){
					params.put("redPacketImgEndTime", date);
				}
				// 校验红包数量是否被领完
				@SuppressWarnings("unchecked")
				Map<String, Object> map_ = (Map<String, Object>) getBaseDao().queryForObject(
						"ImgRedPacketMapper.queryRedPacketMoneyStock", params);
				if (null == map_) {
					params.put("redPacketImgStock", 0);
				} else {
					params.put("redPacketImgStock", stock < 0 ? 0 : stock - 1 );
				}
				int k = getBaseDao().update("ImgRedPacketMapper.updateByExampleSelective", params);
				if (k < 0) {
					output.setCode("-1");
					output.setMsg("抢红包失败");
				}
	...
	public void robRedPacketInfo(InputDTO input, OutputDTO output) throws NzbDataException {
		...
		try {
			...
			Map<String, Object> redPacket = (Map<String, Object>) getBaseDao().queryForObject(
					"ImgRedPacketMapper.queryRedPacketDetail", params);
			...
			if (Optional.ofNullable(zxAppUser).isPresent() && Optional.ofNullable(redPacket).isPresent()) {
				...
				Map<String, Object> one = (Map<String, Object>) getBaseDao().queryForObject(
						"ImgRedPacketMapper.queryRedPacketInfoDetail", params);
				if (Optional.ofNullable(one).isPresent() && StringUtil.isNotEmpty((String) one.get("redPacketImgUserId"))) {
					output.setCode("-1"); // 4
				...
				String answer = (String) params.get("redPacketImgAnswer"); // 判断回答问题答案
				if (answer == null || answer.isEmpty() || !answer.equals(redPacket.get("redPacketImgAnswer"))) {
				...
				Map<String, Object> maps = (Map<String, Object>) getBaseDao().queryForObject(
						"ImgRedPacketMapper.randomSelectRedPacketInfo", redPacket);
				...
				if (Optional.ofNullable(maps).isPresent()) {
					...
					mapp.put("redPacketId", params.get("redPacketImgId"));
					...
					if (time != -1) {
						...
						if (null != appUser) {
							...
							if ("0".equals(redPacket.get("redPacketImgType"))) {
							...
							if (i > 0) {
								...
								if (null != zxAppUser1) {
									if ("0".equals(redPacket.get("redPacketImgType"))) {
										...
										walletMoneyInfo.put("walletInfoFrom", "图文广告红包--来自" + zxAppUser1.get("nickname"));
										getBaseDao().insert("WalletMoneyInfoMapper.saveWalletMoneyInfo", walletMoneyInfo);
									} else {
										// 广告钱包记录
										Map<String, Object> adRecord = new HashMap<>();
										...
										adRecord.put("advertisingInfoFrom", "图文广告红包--来自" + zxAppUser1.get("nickname"));
										getBaseDao().insert("AdvertisingMoneyInfoMapper.saveAdvertisingMoneyInfo", adRecord);
									}
								}
								maps = (Map<String, Object>) getBaseDao().queryForObject("ImgRedPacketMapper.selectRedPacketInfoByInfoId", maps);
								output.setCode("0"); // 3
		...
		try {
			if(Optional.ofNullable(params).isPresent()) {
				...
				List<Map<String,Object>> list = getBaseDao().queryForList(
						"ImgRedPacketMapper.selectRedPacketInfoByRobUserId", params);
				...
				Map<String, Object> maps = (Map<String, Object>) getBaseDao().
						queryForObject("ImgRedPacketMapper.queryRedPacketLog", id);
				if(Optional.ofNullable(maps).isPresent()){
					output.setCode("0");
	...
	public void viewRedPacketInfo(InputDTO input, OutputDTO output) throws NzbDataException {
		...
		try {
			if(Optional.ofNullable(params).isPresent()){
				...
				Map<String, Object> maps = (Map<String, Object>) getBaseDao().queryForObject(
						"ImgRedPacketMapper.selectUserRedPacketInfoAndRedPacketByUserId", params);
				if(Optional.ofNullable(maps).isPresent()){
					List<Map<String, Object>> maps_ = getBaseDao().queryForList(
							"ImgRedPacketMapper.selectRedPacketInfoByRedPacketId", params);
					maps.put("list",maps_);
					output.setItem(maps);
					output.setCode("0");
					output.setMsg("查询成功!");
	...
	public void selectListStockByRedPacketId(InputDTO input, OutputDTO output) throws NzbDataException {
		...
		try {
			...
			List<Map<String, Object>> list = getBaseDao().queryForList(
					"ImgRedPacketMapper.selectListStockByRedPacketId", params);
	...
	public void queryAllRedPacketByAuth(InputDTO input, OutputDTO output) throws NzbDataException {
		...
		try {
			...
			Map<String, Object> zxAppUser =  (Map<String, Object>) getBaseDao().
					queryForObject("MemberMapper.queryMemberDetail", params);
			List<Map<String, Object>> list = getBaseDao().queryForList("ImgRedPacketMapper.queryRedPacketLog", params);
...
//						}
	...
	private OutputDTO checkWalletMoney(InputDTO input, OutputDTO output) {
		Integer redPacketImgCount = (Integer) input.getParams().get("redPacketImgCount"); // 红包个数
		BigDecimal redPacketImgMoneyCount = (BigDecimal) input.getParams().get("redPacketImgMoneyCount"); // 红包金额大小
		Double money = redPacketImgMoneyCount.doubleValue() / redPacketImgCount;
		if (money < 0.2) {
		...
		Map<String, Object> item = (Map<String, Object>) getBaseDao().
				queryForObject("MemberMapper.queryMemberBalanceDetail", input.getParams());
		if (null != item) {
			String type = (String) input.getParams().get("redPacketImgType");
			switch (type) {
				case WALLET_TYPE:
	 ...
	 * 计算手气最佳
	 * @param doubles
	 * @param redPacketImgId
	 * @return
	 */
	private int insertListRedPacketImgInfo(List<Double> doubles, Long redPacketImgId) {
		int i = 0;
		Double maxMoney = getMaxMoney(doubles);
		for (Double num : doubles) {
			Map<String, Object> params = new HashMap<>();
			params.put("redPacketImgInfoId", getSequence()); // 视频信息红包ID
			params.put("redPacketImgId", redPacketImgId); // 视频红包ID
			...
			if(maxMoney.doubleValue() != 0 && num.doubleValue() == maxMoney.doubleValue()){
				...
				i += getBaseDao().insert("ImgRedPacketMapper.insertListRedPacketInfo", params);
				continue;
			} else {
