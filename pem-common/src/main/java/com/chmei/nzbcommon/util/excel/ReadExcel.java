...
/**
 * ClassName:ReadExcel Function: TODO ADD FUNCTION. Date: 2018年7月20日 下午5:21:00
 * 
 * @author LiuYuanTao
 * @version
 * @since JDK 1.7
...
public class ReadExcel {
	...
	/**
	 * 读EXCEL文件，获取信息集合,自定义表头
	 * 
	 * @param fielName
	 * @param set
	 * @return
	 */
	public List<Map<String, Object>> getExcelInfo(MultipartFile mFile, ArrayList<String> list) {
		String fileName = mFile.getOriginalFilename();// 获取文件名
		// List<Map<String, Object>> userList = new LinkedList<Map<String,
		// Object>>();
		try {
			...
			return createExcel(mFile.getInputStream(), isExcel2003, list);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	 ...
	 * @throws IOException
	 */
	public List<Map<String, Object>> createExcel(InputStream is, boolean isExcel2003, ArrayList<String> list) {
		try {
			Workbook wb = null;
			if (isExcel2003) {// 当excel是2003时,创建excel2003
				wb = new HSSFWorkbook(is);
			...
			return readExcelValue(wb, list);// 读取Excel里面客户的信息
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
	...
	/**
	 * 读取Excel里面客户的信息,自定义表头
	 * 
	 * @param wb
	 * @return
	 */
	@SuppressWarnings("deprecation")
	private List<Map<String, Object>> readExcelValue(Workbook wb, ArrayList<String> list) {
		// 得到第一个shell
		...
		if (totalRows > 1 && sheet.getRow(0) != null) {
			this.totalCells = sheet.getRow(0).getPhysicalNumberOfCells();
				for (int i = 0; i < totalCells - listSize; i++) {
					list.add("value" + i + 1);
				}
				...
				if (i < totalCells) {
					...
					if (null != cell) {
						...
						if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
							...
							map.put(list.get(i), name.substring(0, name.length() - 2 > 0 ? name.length() - 2 : 1));// 名称
						} else {
							map.put(list.get(i), cell.getStringCellValue());// 名称
						}
					}
				}
			}
