...
function queryList(pageno,limit){
	...
	Chief.ajax.postJson('/help/queryHelpArchiveList', params, function(data){
...
$('.pull-right').on('click','#add_btn', function (){
	var htmls = Handlebars.compile($("#T_addHelpArchive").html());
	var ht = htmls();
	//弹窗
	dialog("新增", ht);
	initUpload(); //加载上传控件
	formValidate("#addHelpArchiveForm");
});
//新增
function saveHelpArchive(){
	var flag = $("#addHelpArchiveForm").validate().form(); //若全部通过验证则form方法返回true
	if(!flag) {
		return false;
	}
	...
	//发起请求
	Chief.ajax.postJson("/help/saveHelpArchiveInfo", data, function (data) {
		if(data.code == '0'){
...
//调用删除
function delHelpArchive(id) {
	Chief.layer.confirm("是否确认删除！",function() {
		Chief.ajax.postJson('/help/deleteHelpArchiveInfo',{"id":id},function(data){
			if(data.code == '0'){
...
$('.pull-right').on('click','#edit_btn', function (){
	...
	Chief.ajax.postJson('/help/queryHelpArchiveDetail',paramId, function(data) {
		if('0' == data.code){
			var htmls = Handlebars.compile($("#T_editHelpArchive").html());
			//filePaths回写到隐藏域
			...
			dialog("编辑", ht);
			//初始化表单验证规则
			formValidate("#editHelpArchiveForm");
			initUpload(); // 初始化上传组件
			// $("#filePaths").val(data.item.helpArchiveUrl);
			// $("#fileName").text(data.item.helpArchiveUrl.split("#")[1]);
		}else {
 ...
 * 编辑 保存
 */
function updateHelpArchive(){
	var flag = $("#editHelpArchiveForm").validate().form(); //若全部通过验证则form方法返回true
	if(!flag) {
		return false;
	}
	// var filePaths = $("#filePaths").val();
	...
	//发起请求
	Chief.ajax.postJson("/help/updateHelpArchiveInfo", data, function (data) {
		if("0" == data.code){
...
function queryDetail(id) {
	...
	var params = {id: id}
	Chief.ajax.postJson('/help/queryHelpArchiveDetail',params, function(data) {
		if('0' == data.code){
			var htmls = Handlebars.compile($("#T_helpArchiveDetail").html());
			var ht = htmls(data.item);
			dialog("查看详情", ht);
