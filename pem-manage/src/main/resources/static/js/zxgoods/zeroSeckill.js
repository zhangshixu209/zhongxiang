...
function queryList(pageno,limit){
	...
	Chief.ajax.postJson('/seckill/queryGoodsExamineList', params, function(data){
...
$('.pull-right').on('click','#audit_btn', function (){
	...
	Chief.ajax.postJson('/seckill/queryGoodsExamineDetail',paramId, function(data) {
		if('0' == data.code){
...
function updateCashAudit(val){
	...
	//发起请求
	Chief.ajax.postJson("/seckill/authGoodsExamineInfo", data, function (data) {
		if("0" == data.code){
...
$("body").on('click','#offline_btn',function() {
	...
	if (onlineStatus == "1005") {
		...
		Chief.layer.tips('商品已结束！');
		return false;
	} else if (onlineStatus != "1004"){
		Chief.layer.tips('只能下架已上架的商品！');
		return false;
	}
	var param = {
		id: id,
...
function updateOnlineStatus(param){
	Chief.ajax.postJson("/seckill/goodsShelfInfo", param, function (data) {
		if("0" == data.code){
