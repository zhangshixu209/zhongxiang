...
function queryList(pageno,limit){
	...
	Chief.ajax.postJson('/partner/queryOfficialPartnerList', params, function(data){
...
function saveRotationChart(){
	...
	if(isDataNull(filePaths)){
		Chief.layer.tips("请上传LOGO！");
		return false;
	}
	//获取表单数据
	var data = $('#addRotationChartForm').serialize();
	//发起请求
	Chief.ajax.postJson("/partner/saveOfficialPartnerInfo", data, function (data) {
		if(data.code == '0'){
...
function delRotationChart(id) {
	Chief.layer.confirm("是否确认删除！",function() {
		Chief.ajax.postJson('/partner/delOfficialPartnerInfo',{"id":id},function(data){
			if(data.code == '0'){
...
$('.pull-right').on('click','#edit_btn', function (){
	...
	Chief.ajax.postJson('/partner/queryOfficialPartnerDetail',paramId, function(data) {
		if('0' == data.code){
			...
			initUpload(); // 初始化上传组件
			$("#filePaths").val(data.item.partnerLogo);
			$("#fileName").text(data.item.partnerLogo.split("#")[1]);
		} else {
...
function updateRotationChart(){
	...
	//发起请求
	Chief.ajax.postJson("/partner/updateOfficialPartnerInfo", data, function (data) {
		if("0" == data.code){
...
function queryDetail(id) {
	...
	var params = {id: id}
	Chief.ajax.postJson('/partner/queryOfficialPartnerDetail',params, function(data) {
		if('0' == data.code){
