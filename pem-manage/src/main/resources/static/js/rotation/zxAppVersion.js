...
function queryList(pageno,limit){
	...
	Chief.ajax.postJson('/appVersion/queryZxAppVersionList', params, function(data){
...
function saveRotationChart(){
	...
	//发起请求
	Chief.ajax.postJson("/appVersion/saveZxAppVersionInfo", data, function (data) {
		if(data.code == '0'){
...
function bindEvent() {
	...
	$('.ndiv').on('click', 'span.del' ,function(){
		var filePathArrytem = $('#appUrl').val().split("$");
		var span = $(this);
		var $p = span.parent();
		...
		$('#appUrl').val(filePathArrytem.join("$"));
		$p.remove(); //移除这一行
	});
}
...
//    var filePathArry = []; // 存放文件路径和名称
	...
	uploaderS = WebUploader.create({
		...
		fileSingleSizeLimit: 90 * 1024 * 1024, // 单个文件
