...
function changeColor(_this){
	...
	if(_this.parents("table").hasClass("moreCheked")){
		if (check) {
...
//點擊tr
$('body').on('click','tbody tr',function(e,flag) {
	if ($(this).parents('table').attr('date-type') && flag != "checked") {
		return false
	}
...
$('body').on('click', 'tbody tr input[type=checkbox]', function(e) {
	...
	var obj = {"checked":"checked"};
	$(this).parents('tr').trigger("click",["checked"]);
	// return false
})
